...
class UserShopping(ListView):
    ...
    def get_queryset(self):
        queryset = super(UserShopping, self).get_queryset()
...
def my_shopping_detail(request: HttpRequest, order_id):
    ...
    if order is None:
        return Http404('سبد خرید مورد نظر یافت نشد')
    context = {
        'order': order
    }
    return render(request, 'user_panel_module/user_shopping_detail_page.html', context)
